...
int main(int argc, char **argv)
{
    ...
    if (argc < 7) {
        printf("%s dev_node r/w size round enable_hugepage ep_addr [val];  "
    ...
    virtual_mmap = mmap(0, 0x100000, PROT_READ | PROT_WRITE, MAP_SHARED, fd,
                        (0xe0000000 + 0x1000000));
    if (!virtual_mmap) {
    ...
    printf("round=%d size=0x%x val=0x%x  virtual_src=%p virtual_dst=%p "
           "ep_addr=0x%x. \n",
        ...
        if ((flag == 0x0) || (flag == 0x3)) {
            ...
            for (i = 0; i < size; i++) {
                if (*(virtual_src + i) != *(virtual_mmap + i)) {
                    printf("rc2ep compare failed, size=0x%x i=0x%x src=0x%x "
                           "mmap=0x%x\n",
            ...
            if (i == size)
                printf("round:%d rc2ep compare successfully, size=0x%x, "
                       "mmap=0x%x\n",
        ...
        if ((flag == 0x1) || (flag == 0x3)) {
            ...
            for (i = 0; i < size; i++) {
                if (*(virtual_dst + i) != *(virtual_mmap + i)) {
                    printf("ep2rc compare failed, size=0x%x i=0x%x dst=0x%x "
                           "mmap=0x%x\n",
            ...
            if (i == size)
                printf("round:%d ep2rc compare successfully, size=0x%x, "
                       "dst=0x%x mmap=0x%x\n",
                       save_round - round, size, *(virtual_dst + i - 1),
                       *(virtual_mmap + size - 1));
        }
        printf("\n\n");
    }
