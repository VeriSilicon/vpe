...
#define WRAPPER_CLK_EN                                                      \
    (SFT_EN_VC8000D_PCLK | FT_EN_VC8000D_ACLK | SFT_EN_VC8000D_ACLK_ASYNC | \
     SFT_EN_L2RW_VCD_DEC400_F1_WRAPPER_PCLK |                               \
...
static const unsigned int vcd_cores[VCD_MAX_CORES][3] = {
...
static const char *vc8000d_interrupt_info[4][5] = {
    { "s0_vcd_a_vc8000d_top_int", "s0_vcd_a_sram_ecc_e2_int",
      "s0_vcd_a_sram_ecc_e1_int", "s0_vcd_a_l2rw_vcd_int",
      "s0_vcd_a_dec400_f1_int" },
    { "s1_vcd_a_vc8000d_top_int", "s1_vcd_a_sram_ecc_e2_int",
      "s1_vcd_a_sram_ecc_e1_int", "s1_vcd_a_l2rw_vcd_int",
      "s1_vcd_a_dec400_f1_int" },
    { "s0_vcd_b_vc8000d_top_int", "s0_vcd_b_sram_ecc_e2_int",
      "s0_vcd_b_sram_ecc_e1_int", "s0_vcd_b_l2rw_vcd_int",
      "s0_vcd_b_dec400_f1_int" },
    { "s1_vcd_b_vc8000d_top_int", "s1_vcd_b_sram_ecc_e2_int",
      "s1_vcd_b_sram_ecc_e1_int", "s1_vcd_b_l2rw_vcd_int",
      "s1_vcd_b_dec400_f1_int" }
};

const char *core_status[5] = { "idle", "reserve", "enable", "rcv_irq",
                               "chk_irq" };
...
static void vcd_enable_clock(struct vc8000d_t *tvcd, int id)
{
    ...
    case 3:
        ...
        break;
    default:
        trans_dbg(tvcd->tdev, TR_ERR, "vc8000d: %s core:%d error.\n", __func__,
                  id);
        break;
    }
}
...
int adjust_vcd_pll(struct cb_tranx_t *tdev, u32 slice_id)
{
    ...
    case 4:
        ...
        break;
    default:
        trans_dbg(tdev, TR_ERR, "PLL: %s adjust:%d error, slice_id:%d.\n",
...
static int vcd_a_sft_reset(struct cb_tranx_t *tdev, int core)
{
    ...
    trans_dbg(tdev, TR_ERR, "vc8000d: %s core:%d %s.\n", __func__, core,
              ((val & 0x3) == 0x1) ? "done" : "failed");

    return ((val & 0x3) == 0x1) ? 0 : -EFAULT;
}

/* software release vc8000d core a */
static int vcd_a_sft_release(struct cb_tranx_t *tdev, void __iomem *vcdbase,
                             int core)
{
    unsigned int val;
    unsigned int round = RESET_ROUND;
...
static int vcd_b_sft_reset(struct cb_tranx_t *tdev, int core)
{
    ...
    trans_dbg(tdev, TR_ERR, "vc8000d: %s core:%d %s.\n", __func__, core,
              ((val & 0xc) == 0x4) ? "done" : "failed");

    return ((val & 0xc) == 0x4) ? 0 : -EFAULT;
}

/* software release vc8000d core b */
static int vcd_b_sft_release(struct cb_tranx_t *tdev, void __iomem *vcdbase,
                             int core)
{
    unsigned int val;
    unsigned int round = RESET_ROUND;
    ...
    for (i = 0; i < VCD_MAX_CORES; i++) {
        ...
        if (IS_VC8000D(hwid)) {
            ...
            trans_dbg(tvcd->tdev, TR_DBG,
                      ...
                      tvcd->core[i].hwbase, readl(tvcd->core[i].hwregs));
        } else {
...
static void read_core_config(struct vc8000d_t *tvcd)
{
    ...
    for (c = 0; c < tvcd->cores; c++) {
        ...
        tmp  = (reg & mask);
        if (tmp & (1 << DWL_VP8_E))
            trans_dbg(tvcd->tdev, TR_DBG, "vc8000d: core[%d] has VP8\n", c);
        if (tmp & (1 << DWL_VP7_E))
            trans_dbg(tvcd->tdev, TR_DBG, "vc8000d: core[%d] has VP7\n", c);
        if (tmp & (1 << DWL_WEBP_E))
            trans_dbg(tvcd->tdev, TR_DBG, "vc8000d: core[%d] has WebP\n", c);
        tvcd->core_format[c] |= tmp ? 1 << DWL_CLIENT_TYPE_VP8_DEC : 0;

        tmp = (reg >> DWL_AVS_E) & 0x01;
        if (tmp)
            trans_dbg(tvcd->tdev, TR_DBG, "vc8000d: core[%d] has AVS\n", c);
        tvcd->core_format[c] |= tmp ? 1 << DWL_CLIENT_TYPE_AVS_DEC : 0;

        tmp = (reg >> DWL_RV_E) & 0x03;
        if (tmp)
            trans_dbg(tvcd->tdev, TR_DBG, "vc8000d: core[%d] has RV\n", c);
        ...
        if (tmp)
            trans_dbg(tvcd->tdev, TR_DBG, "vc8000d: core[%d] has HEVC\n", c);
        tvcd->core_format[c] |= tmp ? 1 << DWL_CLIENT_TYPE_HEVC_DEC : 0;

        tmp = (reg >> DWL_VP9_E) & 0x07;
        if (tmp)
            trans_dbg(tvcd->tdev, TR_DBG, "vc8000d: core[%d] has VP9\n", c);
        ...
        if (tmp)
            trans_dbg(tvcd->tdev, TR_DBG, "vc8000d: core[%d] has PP\n", c);
        tvcd->core_format[c] |= tmp ? 1 << DWL_CLIENT_TYPE_PP : 0;

        if ((c == 0) || (c == 2)) {
            tvcd->core_format[c] |= 1 << DWL_CLIENT_TYPE_ST_PP;
            trans_dbg(tvcd->tdev, TR_DBG, "vc8000d: core[%d] has ST_PP\n", c);
        }
    }
}

/* get a core supported this format */
static int find_a_core(struct vc8000d_t *tvcd, struct file *filp,
                       unsigned int format)
{
    int c;
    int id = -1;
...
static void vcd_kickoff_next_task(struct vc8000d_t *tvcd, int id)
{
    ...
    if (!list_empty(&tvcd->list_live)) {
        ...
        {
            if ((f->reserved == 0) &&
                (core_has_format(tvcd->core_format, id, f->format))) {
                find                = 1;
                f->reserved         = 1;
    ...
    if ((find == 0) && !list_empty(&tvcd->list_vod)) {
        trans_dbg(tvcd->tdev, TR_DBG, "vc8000d: list_vod count %d.\n",
                  tvcd->vod_count);
 ...
 */
static int vc8000d_reserve_core(struct vc8000d_t *tvcd, struct file *filp,
                                unsigned int format, char task_priority)
{
    int id = -1;
    int i;
    int ret;
    ...
    if (WARN_ON(task_priority > TASK_VOD)) {
        trans_dbg(tvcd->tdev, TR_ERR, "vc8000d: task_priority:%d error\n",
                  task_priority);
        return -EINVAL;
    }
    ...
    if (id != -1) {
        ...
        if (id >= tvcd->cores)
            trans_dbg(tvcd->tdev, TR_ERR, "vc8000d: %s, core >= tvcd->cores\n",
                      __func__);
    }
    ...
    if (tvcd->core[id].core_status != IDLE_FLAG) {
        trans_dbg(tvcd->tdev, TR_ERR, "vc8000d: %s core_%d_status:0x%x error\n",
...
static void vc8000d_release_core(struct vc8000d_t *tvcd, int id, int mode,
                                 struct file *filp)
{
    u32 status;
    int c;
    ...
    if (tvcd->core[id].filp == NULL) {
        trans_dbg(tvcd->tdev, TR_ERR, "vc8000d: %s core:%d is not reserve\n",
                  __func__, id);
        return;
    }
    ...
    if (mode == ABNORM_EXIT) {
        ...
        trans_dbg(tvcd->tdev, TR_ERR,
                  "vc8000d: %s, abnorm exit, wait core_%d 50ms for ip done, "
                  "core status:%d\n",
                  __func__, id, tvcd->core[id].core_status);
    }

    if (tvcd->core[id].core_status != CHK_IRQ_FLAG) {
        trans_dbg(tvcd->tdev, TR_ERR, "vc8000d: %s, core_%d_status:%s",
                  __func__, id, core_status[tvcd->core[id].core_status]);
    }
    ...
    if (status & VCD_DEC_E) {
        trans_dbg(tvcd->tdev, TR_ERR,
                  "vc8000d: core:%d status is enabled, force reset; "
                  "status=0x%x, reg[21]=0x%x reg[3]=0x%x, filp=%p\n",
                  id, status,
                  readl(tvcd->core[id].hwregs + VCD_VLC_CODE_LEN_OFF),
                  readl(tvcd->core[id].hwregs + VCD_CTRL_REG3_OFF), filp);
        /* abort decoder */
        status |= VCD_DEC_ABORT | VCD_DEC_IRQ_DISABLE;
    ...
    if (id == 0) {
        ...
        tvcd->loading[0].time_cnt += ktime_to_us(
            ktime_sub(tvcd->loading[0].tv_e, tvcd->loading[0].tv_s));
    } else if (id == 2) {
        tvcd->loading[1].tv_e = ktime_get();
        tvcd->loading[1].time_cnt += ktime_to_us(
            ktime_sub(tvcd->loading[1].tv_e, tvcd->loading[1].tv_s));
    }
...
static ssize_t dec_util_show(struct device *dev, struct device_attribute *attr,
                             char *buf)
{
    ...
    average_loading =
        (tvcd->loading[0].time_cnt_saved + tvcd->loading[1].time_cnt_saved) *
        100 / (tvcd->loading[0].total_time + tvcd->loading[1].total_time);
                                    ...
                                    struct device_attribute *attr, char *buf)
{
    ...
    for (i = 0; i < 4; i++) {
        pos += sprintf(buf + pos, "core:%d  status:%s\n", i,
                       core_status[tvcd->core[i].core_status]);
    }
                                    ...
                                    struct device_attribute *attr, char *buf)
{
    ...
    for (i = 0; i < 4; i++) {
        pos +=
            sprintf(buf + pos, "core:%d  count:%d\n", i, tvcd->core[i].rsv_cnt);
    }
...
static struct attribute *trans_dec_sysfs_entries[] = {
    &dev_attr_dec_util.attr, &dev_attr_dec_core_status.attr,
    &dev_attr_dec_reserve_cnt.attr, NULL
};
    ...
    for (id = 0; id < VCD_MAX_CORES; id++) {
        if (tvcd->core[id].filp == filp) {
            trans_dbg(tvcd->tdev, TR_ERR,
                      ...
                      id, filp);
            //			vc8000d_release_core(tvcd, id, ABNORM_EXIT,
            //filp);
        }
    }
}

static int check_dec_irq(struct vc8000d_t *tvcd, const struct file *filp,
                         u32 id)
{
    int rdy = 0;
...
long wait_dec_ready(struct vc8000d_t *tvcd, const struct file *filp, u32 id)
{
    ...
    ret = wait_event_interruptible_timeout(tvcd->dec_wait_queue,
                                           check_dec_irq(tvcd, filp, id),
                                           VC8000D_TIMEOUT);
    if (!ret) { // timeout
        trans_dbg(tvcd->tdev, TR_ERR,
                  "vc8000d: %s core:%d timeout, hw_status:0x%x\n", __func__, id,
                  readl(tvcd->core[id].hwregs + 4));
        ret = -EFAULT;
...
static int check_core_irq(struct vc8000d_t *tvcd, const struct file *filp,
                          int *core_id)
{
    u32 format;
    int rdy = 0, id = 0;
    ...
    for (id = 0; id < tvcd->cores; id++) {
        if ((tvcd->core[id].filp == filp) && (tvcd->core[id].irq_rcvd)) {
            ...
            if (format & 0xF8000000) /* if current decoding is not h264(0:h264),
                                        continue */
                continue;
            ...
            *core_id = id;
            ...
            if (tvcd->core[id].core_status != RCVD_IRQ_FLAG) {
                trans_dbg(tvcd->tdev, TR_ERR, "vc8000d: %s, core_%d_status:%s",
                          __func__, id,
                          core_status[tvcd->core[id].core_status]);
            }
            tvcd->core[id].chk_irq_cnt++;
            tvcd->core[id].core_status = CHK_IRQ_FLAG;
            break;
...
irqreturn_t vcd_isr(int index, void *data)
{
    ...
    if (RSV_FLAG != core->core_status) {
        if (IDLE_FLAG != core->core_status) {
            ...
            trans_dbg(tdev, TR_ERR,
                      "vc8000d: %s, core_%d_status:0x%x err, hw_status:0x%x\n",
    ...
    if (tvcd->core[index].filp == NULL) {
        trans_dbg(tvcd->tdev, TR_ERR,
                  "vc8000d: %s index:%d, owner is null, fatal error, "
                  "hw_status=0x%x, tdev=%p.\n",
    ...
    if (irq_status_dec & ((1 << 0) | (1 << 2) | (1 << 3) | (1 << 5)))
        trans_dbg(tvcd->tdev, TR_ERR,
                  ...
                  index, irq_status_dec, tvcd->tdev);
    ...
    if (handled) {
        ...
        if (chip_int_2_stus & (1 << (index * VCD_IRQ_CNT))) {
            trans_dbg(tvcd->tdev, TR_ERR,
                      "vc8000d: not done interrupt! index=%d global_sta:0x%x "
                      "dec_status:0x%x\n",
                      index, chip_int_2_stus, irq_status_dec);
        }
        for (i = 0; i < 5; i++) {
            if (chip_int_2_stus & (1 << (index * VCD_IRQ_CNT + i))) {
                trans_dbg(tvcd->tdev, TR_ERR, "vc8000d: receive irq is: %s.\n",
    ...
    for (i = 0; i < VCD_MAX_CORES; i++) {
        ...
        if (tvcd->core[i].irq != -1) {
            ...
            result = request_irq(tvcd->core[i].irq, unify_isr,
                                 IRQF_SHARED | IRQF_NO_THREAD, "vc8000d",
                                 (void *)tdev);
            if (result != 0) {
                if (result == -EINVAL) {
                    trans_dbg(tdev, TR_ERR, "vc8000d: Bad IRQ:%d or handler\n",
                              tvcd->core[i].irq);
                } else if (result == -EBUSY) {
                    trans_dbg(tdev, TR_ERR, "vc8000d: IRQ:%d busy\n",
                              tvcd->core[i].irq);
                }
                goto out_irq_free;
            }
        } else {
            trans_dbg(tdev, TR_DBG, "vc8000d: cord:%d IRQ not in use!\n", i);
        }
    }
    return 0;
...
static void vc8000d_free_irq(struct cb_tranx_t *tdev)
{
    ...
    for (n = 0; n < VCD_MAX_CORES; n++) {
        if (tvcd->core[n].irq != -1) {
            trans_dbg(tdev, TR_DBG, "vc8000d: free irq tvcd->irq[%d]:%d.\n", n,
                      tvcd->core[n].irq);
            free_irq(tvcd->core[n].irq, (void *)tdev);
        }
    }
...
static void init_clk_rst(struct vc8000d_t *tvcd)
{
    ...
    for (i = 0; i < VCD_MAX_CORES; i++) {
        wrapper_reg =
            tvcd->tdev->bar2_virt + vcd_cores[i][0] + VCD_WRAPPER_REG_OFF;
...
int vc8000d_init(struct cb_tranx_t *tdev)
{
    ...
    for (i = 0; i < tvcd->cores; i++) {
        ...
        trans_dbg(tdev, TR_DBG, "vc8000d: synth_cfg:0x%x synth_cfg_2:0x%x\n",
                  tvcd->vcd_cfg[i].hantrodec_synth_cfg,
                  tvcd->vcd_cfg[i].hantrodec_synth_cfg_2);
        trans_dbg(tdev, TR_DBG, "vc8000d: synth_cfg_3:0x%x fuse_cfg:0x%x\n",
                  tvcd->vcd_cfg[i].hantrodec_synth_cfg_3,
                  tvcd->vcd_cfg[i].hantrodec_fuse_cfg);
        trans_dbg(tdev, TR_DBG, "vc8000d: pp_synth_cfg:0x%x pp_cfg_stat:0x%x\n",
                  tvcd->vcd_cfg[i].hantrodecpp_synth_cfg,
                  tvcd->vcd_cfg[i].hantrodecpp_cfg_stat);
                   ...
                   struct cb_tranx_t *tdev)
{
    ...
    case CB_TRANX_VCD_CORE_CNT:
        ...
        trans_dbg(tdev, TR_DBG, "vc8000d: hantrodec_data->cores:%d\n",
                  tvcd->cores);
        break;
    ...
    case CB_TRANX_VCD_GET_CFG:
        ...
        if (cfg.core_id >= tvcd->cores) {
            trans_dbg(tdev, TR_ERR, "vc8000d: get_cfg id:%d error\n",
                      cfg.core_id);
            return -EFAULT;
        }
        ...
        if (ret) {
            trans_dbg(tdev, TR_ERR, "vc8000d: get_cfg copy_to_user failed.\n");
            return -EFAULT;
        }
        break;
    case CB_TRANX_VCD_RESERVE:
        ...
        if (ret) {
            trans_dbg(tdev, TR_ERR, "vc8000d: reserve copy_from_user failed\n");
            return -EFAULT;
        }
        core_id =
            vc8000d_reserve_core(tvcd, filp, info.format, info.task_priority);
        return core_id;
    case CB_TRANX_VCD_RELEASE:
        __get_user(id, (u32 *)argp);
        trans_dbg(tdev, TR_DBG, "vc8000d: Release core:%d\n", id);
        if (id >= tvcd->cores || tvcd->core[id].filp != filp) {
            trans_dbg(tdev, TR_ERR, "vc8000d: bogus DEC release, core:%d.\n",
                      id);
            return -EFAULT;
        }
    ...
    case CB_TRANX_VCD_WAIT_DONE:
        ...
        if (ret == 0)
            trans_dbg(tdev, TR_DBG, "vc8000d: core:%d ready,\n", core_id);
    ...
    case CB_TRANX_VCD_WAIT_CORE_DONE: /*wait for interrupt from specified core*/
        ...
        break;
    default:
        trans_dbg(tdev, TR_ERR, "vc8000d: %s, cmd:0x%x is error.\n", __func__,
                  cmd);
        ret = -EINVAL;
    }
