...
static void enc_kickoff_next_task(struct encoder_t *tenc, u32 core)
{
    ...
    if (!list_empty(&tenc->live_list)) {
        trans_dbg(tenc->tdev, TR_DBG, "encoder: list_live count %d.\n",
                  tenc->live_count);
        ...
        list_for_each_entry_safe(f, t, &tenc->live_list, rsv_list)
        {
            if (f->reserved == 0) {
                ...
                tenc->live_count--;
                trans_dbg(tenc->tdev, TR_DBG, "encoder: get valid element.\n");
                break;
            }
        }
    }
    ...
    if ((find == 0) && !list_empty(&tenc->vod_list)) {
        trans_dbg(tenc->tdev, TR_DBG, "encoder: list_vod count %d.\n",
                  tenc->vod_count);
...
int reserve_encoder(struct cb_tranx_t *tdev, u32 *core, u32 task_priority)
{
    ...
    if (task_priority > TASK_VOD) {
        trans_dbg(tdev, TR_ERR, "encoder: task_priority:%d error\n",
                  task_priority);
        return -EINVAL;
    }
    ...
    if (success == 1) {
        ...
        trans_dbg(tdev, TR_DBG, "encoder: get a idle core:%d, priority:0x%x.\n",
                  *core, task_priority);
    } else {
        ...
        if (wait_event_interruptible(tenc->wait_queue, new->reserved)) {
            ...
            spin_unlock(&tenc->enc_lock);
            trans_dbg(tdev, TR_ERR, "encoder: reserve wait terminated.\n");
            return -ERESTARTSYS;
        }
...
int release_encoder(struct cb_tranx_t *tdev, u32 core)
{
    ...
    tenc = tdev->modules[TR_MODULE_ENCODER];
    if (core >= tenc->cores) {
        trans_dbg(tdev, TR_ERR, "encoder: release core_id:%d is error.\n",
                  core);
        return -EFAULT;
    }
    ...
    if (core == 0) {
        ...
        tenc->loading[0].time_cnt += ktime_to_us(
            ktime_sub(tenc->loading[0].tv_e, tenc->loading[0].tv_s));
    } else {
        tenc->loading[1].tv_e = ktime_get();
        tenc->loading[1].time_cnt += ktime_to_us(
            ktime_sub(tenc->loading[1].tv_e, tenc->loading[1].tv_s));
    }
...
static ssize_t enc_util_show(struct device *dev, struct device_attribute *attr,
                             char *buf)
{
    ...
    average_loading =
        (tenc->loading[0].time_cnt_saved + tenc->loading[1].time_cnt_saved) *
        100 / (tenc->loading[0].total_time + tenc->loading[1].total_time);
                                    ...
                                    struct device_attribute *attr, char *buf)
{
    ...
    for (i = 0; i < 2; i++) {
        pos += sprintf(buf + pos, "core:%d  status:%s\n", i,
                       core_status[tenc->codec[i].core_status]);
    }
                                    ...
                                    struct device_attribute *attr, char *buf)
{
    ...
    for (i = 0; i < 2; i++) {
        pos += sprintf(buf + pos, "core:%d  count:%d\n", i,
                       tvce->core[i].rsv_cnt + tbigsea->core[i].rsv_cnt);
    }
...
static int enc_sft_reset(struct cb_tranx_t *tdev, int core)
{
    ...
    if (!done) {
        trans_dbg(tdev, TR_ERR, "encoder: %s core:%d failed.\n", __func__,
                  core);
        return -EFAULT;
    }
...
int adjust_enc_pll(struct cb_tranx_t *tdev, u32 slice_id, int type)
{
    ...
    if (type == TR_PLL_BIGSEA) {
        ...
        trans_dbg(tdev, TR_ERR,
                  ...
                  slice_id, tdev->clock_adjust, type);
        ret = -EFAULT;
    }
...
static struct attribute *trans_enc_sysfs_entries[] = {
    &dev_attr_enc_util.attr,        &dev_attr_enc_s0_clk.attr,
    &dev_attr_enc_s1_clk.attr,      &dev_attr_enc_core_status.attr,
    &dev_attr_enc_reserve_cnt.attr, NULL
};
...
int encoder_init(struct cb_tranx_t *tdev)
{
    ...
    if (!tenc) {
        trans_dbg(tdev, TR_ERR, "encoder: kmalloc encoder struct failed.\n");
        return -EFAULT;
    }
    ...
    if (!buf) {
        trans_dbg(tdev, TR_ERR, "encoder: kmalloc encoder taskq failed.\n");
        goto free_dev;
    }
    for (i = 0; i < TR_MAX_LIST; i++)
        tenc->taskq[i] = buf + i;
