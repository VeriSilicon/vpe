...
#define RESET_STATUS                                       \
    (SYS_CON_CSYSREQ | SYS_CON_CACTIVE | SYS_CON_CSYSACK | \
     SYS_CON_SLV_CSYSREQ | SYS_CON_SLV_CACTIVE | SYS_CON_SLV_CSYSACK)

#define RELEASE_STATUS                                         \
    (SYS_CON_CSYSREQ | SYS_CON_CSYSACK | SYS_CON_SLV_CSYSREQ | \
     SYS_CON_SLV_CSYSACK)
...
char *error_info[64] = { "thermal_event_low",
                         "thermal_event_high",
                         "bus_error",
                         "pll_adjust_error",
                         "mem_repair_error",
...
MODULE_PARM_DESC(fw, "load zsp firmware flag,1:load; 0:not load; default is "
                     "1.");

/* check firmware heartbeat,1:check; 0:not check */
unsigned int hb = 0;
module_param(hb, uint, 0644);
MODULE_PARM_DESC(hb, "check firmware heartbeat,1:check; 0:not check; default "
                     "is 0.");
...
static void parse_err_id(unsigned long error_id, struct hwm_t *thwm)
{
    ...
    for (i = 0; i < 64; i++)
        if ((error_id >> i) & 0x1) {
            ...
            trans_dbg(thwm->tdev, TR_ERR, "hwm: ZSP_REP: %s.\n", error_info[i]);
        }
}
...
static int reset_zsp(struct hwm_t *thwm)
{
    ...
    if (val & RESET_STATUS) {
        trans_dbg(thwm->tdev, TR_ERR, "hwm: check power status_1 timeout\n");
        goto out;
    }
    ...
    if ((val & RELEASE_STATUS) != RELEASE_STATUS) {
        trans_dbg(thwm->tdev, TR_ERR, "hwm: check power status_2 timeout\n");
        goto out;
    }
    ccm_write(thwm->tdev, SC_RST_CON_STUS,
              (ZSP_CORE_NO_SFT_RST | ZSP_EXTSYS_NO_SFT_RST |
               ZSP_SUBSYSTEM_SOFT_RESET));
    ret = 0;
...
static int load_firmware(struct hwm_t *thwm)
{
    ...
    if (request_firmware(&fw, FIRMWARE, &thwm->tdev->pdev->dev)) {
        trans_dbg(thwm->tdev, TR_ERR, "hwm: request_firmware failed.\n");
        goto out;
    }
    ...
    while (time_before(jiffies, delay)) {
        ...
        if ((val_dir == SIGNATURE_TO_RC) && (val_st == DONE)) {
            trans_dbg(thwm->tdev, TR_INF, "hwm: load firmware successfully.\n");
            ret = 0;
            goto out_release_fw;
        }
...
static int send_info_to_zsp(struct hwm_t *thwm, void *data)
{
    ...
    if ((host_mem->event_ID + host_mem->error_ID) == 0) {
        ...
        trans_dbg(thwm->tdev, TR_ERR, "hwm: %s, but ZSP not respond.\n",
                  __func__);
        ret = -EFAULT;
    }
                                            ...
                                            const char *buf, size_t count)
{
    ...
    if ((threshold < 0) || (threshold > 125)) {
        trans_dbg(tdev, TR_ERR, "core: %s, thereshold range is [0 - 125].\n",
                  __func__);
        return -1;
    }
...
static ssize_t zsp_ver_show(struct device *dev, struct device_attribute *attr,
                            char *buf)
{
    struct cb_mail_box_f2d *fw_info;
    struct cb_tranx_t *tdev = dev_get_drvdata(dev);
...
static ssize_t ddr_ver_show(struct device *dev, struct device_attribute *attr,
                            char *buf)
{
    struct cb_mail_box_f2d *fw_info;
    struct cb_tranx_t *tdev = dev_get_drvdata(dev);
...
static ssize_t pvt0_mv_show(struct device *dev, struct device_attribute *attr,
                            char *buf)
{
    struct cb_mail_box_f2d *fw_info;
    struct cb_tranx_t *tdev = dev_get_drvdata(dev);
...
static ssize_t pvt1_mv_show(struct device *dev, struct device_attribute *attr,
                            char *buf)
{
    struct cb_mail_box_f2d *fw_info;
    struct cb_tranx_t *tdev = dev_get_drvdata(dev);
...
static ssize_t pvt2_mv_show(struct device *dev, struct device_attribute *attr,
                            char *buf)
{
    struct cb_mail_box_f2d *fw_info;
    struct cb_tranx_t *tdev = dev_get_drvdata(dev);
...
static ssize_t pvt3_mv_show(struct device *dev, struct device_attribute *attr,
                            char *buf)
{
    struct cb_mail_box_f2d *fw_info;
    struct cb_tranx_t *tdev = dev_get_drvdata(dev);
...
int hw_monitor_register_irq(struct cb_tranx_t *tdev)
{
    ...
    ret = request_irq(thwm->irq, unify_isr, IRQF_SHARED | IRQF_NO_THREAD,
                      "hw_monitor", tdev);
    if (ret)
