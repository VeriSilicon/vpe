...
#define MISC_IP_TOTAL_CORES                                               \
    (VC8000D_CORES + VC8000E_CORES + BIGSEA_CORES + L2CACH_VCD_CORES +    \
     L2CACH_VCE_CORES + F1_CORES + F2_CORES + F3_CORES + F4_TCACH_CORES + \
...
#define MISC_IP_TOTAL_IOSIZE                                               \
    (VC8000D_CORES * VC8000D_IOSIZE + VC8000E_CORES * VC8000E_IOSIZE +     \
     BIGSEA_CORES * BIGSEA_IOSIZE + L2CACH_VCD_CORES * L2CACH_VCD_IOSIZE + \
     L2CACH_VCE_CORES * L2CACH_VCE_IOSIZE + F1_CORES * F1_IOSIZE +         \
     F2_CORES * F2_IOSIZE + F3_CORES * F3_IOSIZE +                         \
     F4_TCACH_CORES * F4_TCACH_IOSIZE + F4_DTRC_CORES * F4_DTRC_IOSIZE +   \
...
static const u32 vc8000d[VC8000D_CORES][2] = {
...
static const u32 vc8000e[VC8000E_CORES][2] = {
    { S0_VC8000E_OFF, VC8000E_IOSIZE },
    { S1_VC8000E_OFF, VC8000E_IOSIZE },
};
static const u32 bigsea[BIGSEA_CORES][2] = {
    { S0_BIGSEA_OFF, BIGSEA_IOSIZE },
    { S1_BIGSEA_OFF, BIGSEA_IOSIZE },
};
static const u32 l2cach_vcd[L2CACH_VCD_CORES][2] = {
...
static inline void check_bar2_addr(struct cb_tranx_t *tdev, void __iomem *addr,
                                   int ip_id)
{
    if ((addr < tdev->bar2_virt) || (addr > tdev->bar2_virt_end))
        trans_dbg(tdev, TR_ERR,
                  "%s ip_id=%d bar2_start=0x%p bar2_end=0x%p addr=0x%p\n",
                  __func__, ip_id, tdev->bar2_virt, tdev->bar2_virt_end, addr,
                  ip_id);
}
...
static int tcache_buf_init(struct cb_tranx_t *tdev, dma_addr_t src_add,
                           dma_addr_t dst_add, u32 size, u32 id)
{
...
int tcache_init(struct cb_tranx_t *tdev)
{
    ...
    kvir_dma = dma_alloc_coherent(&tdev->pdev->dev, dma_size, &bus, GFP_KERNEL);
    if (!kvir_dma) {
        trans_dbg(tdev, TR_ERR, "tcache: dma_alloc_coherent failed.\n");
        goto out;
    }
...
static void check_core_status(struct cb_tranx_t *tdev, int ip_id, int core_id,
                              char *fun_name, int reg_id)
{
    ...
    if (ip_id == VC8000D_ID) {
        ...
        if (video_core->core_status == IDLE_FLAG) {
            trans_dbg(tdev, TR_ERR,
                      "misc_ip: %s-%s vc8000d core_id=%d reg_id:%d "
                      "core_status=%d\n",
                      __func__, fun_name, core_id, reg_id,
                      video_core->core_status);
        }
    }
    if (ip_id == VC8000E_ID) {
        ...
        if (video_core->core_status == IDLE_FLAG) {
            trans_dbg(tdev, TR_ERR,
                      "misc_ip: %s-%s vc8000e core_id=%d reg_id:%d "
                      "core_status=%d\n",
                      __func__, fun_name, core_id, reg_id,
                      video_core->core_status);
        }
    ...
    if (ip_id == BIGSEA_ID) {
        ...
        if (video_core->core_status == IDLE_FLAG) {
            trans_dbg(tdev, TR_ERR,
                      "misc_ip: %s-%s bigsea core_id=%d reg_id:%d "
                      "core_status=%d\n",
                      __func__, fun_name, core_id, reg_id,
                      video_core->core_status);
        }
...
int misc_ip_init(struct cb_tranx_t *tdev)
{
    ...
    if (!tmisc) {
        trans_dbg(tdev, TR_ERR, "misc_ip: %s kmalloc bigseacodec failed\n",
                  __func__);
        goto out;
    }
                   ...
                   struct cb_tranx_t *tdev)
{
    ...
    if (copy_from_user(&ipdesc, argp, sizeof(struct ip_desc))) {
        trans_dbg(tdev, TR_ERR, "misc_ip: %s copy from user failed\n",
                  __func__);
        return -EFAULT;
    }
    ...
    case CB_TRANX_PUSH_REGS:
        ...
        break;
    default:
        trans_dbg(tdev, TR_ERR, "misc_ip: %s, cmd:0x%x error.\n", __func__,
                  cmd);
        ret = -EINVAL;
    }
...
int enable_all_pll(struct cb_tranx_t *tdev)
{
    ...
    for (i = 0; i <= 9; i++) {
        ...
        if ((val & 0x80000000) == 0x80000000) {
            ...
            trans_dbg(tdev, TR_ERR, "core: enable pll_%d failed.\n", i);
            return -EFAULT;
        }
    }
