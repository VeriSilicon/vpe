...
static inline void pcie_write(struct cb_tranx_t *tdev, unsigned int off,
                              unsigned int val)
{
    writel((val), ((off) + tdev->bar0_virt));
}

static inline unsigned int pcie_read(struct cb_tranx_t *tdev, unsigned int off)
{
...
static struct attribute *trans_pcie_sysfs_entries[] = {
    &dev_attr_link_status.attr, &dev_attr_bus_id.attr, NULL
};
...
int cb_pci_init(struct cb_tranx_t *tdev)
{
    ...
    if (ret) {
        trans_dbg(tdev, TR_ERR, "pcie: Unable to Enable PCIe device, ret:%d.\n",
                  ret);
        goto out;
    }
    ...
    if (pci_set_dma_mask(tdev->pdev, DMA_BIT_MASK(64)) ||
        ...
        if (pci_set_dma_mask(tdev->pdev, DMA_BIT_MASK(32)) ||
            ...
            trans_dbg(tdev, TR_ERR, "pcie: No suitable DMA available.\n");
    ...
    if (ret == MSI_NUM) {
        trans_dbg(tdev, TR_DBG, "pcie: Have requested msi irq number:%d.\n",
                  ret);
    } else {
        trans_dbg(tdev, TR_ERR, "pcie: pci_alloc_irq_vectors failed,ret=%d.\n",
                  ret);
        goto out_unmap_bar2;
    }
...
static int switch_ddr_mapping(struct cb_tranx_t *tdev, int ddr_id)
{
                  ...
                  struct cb_tranx_t *tdev)
{
    ...
    case CB_TRANX_GET_BARADDR:
        ...
        if (copy_to_user(argp, &bar_inf, sizeof(bar_inf))) {
            trans_dbg(tdev, TR_ERR, "pcie: GET_BARADDR copy_to_user failed.\n");
            return -EFAULT;
        }
        break;
    case CB_TRANX_SW_DDR_MAPPING:
        ...
        break;
    default:
        trans_dbg(tdev, TR_ERR, "pcie: %s, cmd:0x%x is error.\n", __func__,
                  cmd);
        ret = -EINVAL;
    }
