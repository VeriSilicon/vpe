...
int main(int argc, char **argv) {
	...
	if(argc == 4)
	{
		...
		size += MAP_MASK;
		size &= ~MAP_MASK;
//		printf("len=0x%x size=0x%x s=0x%x e=0x%x \n",len,size,target,strtoul(argv[2], 0, 0));
	}
	else
		size = MAP_SIZE;
...
//		printf("Value at address %p: 0x%X\n", virt_addr, read_result);	
	...
	if(argc == 4) 
	{
		...
		for(i=0;i<=len/4;i++){
			printf("0x%08X -- 0x%08X  \n",target+i*4, *((unsigned int *) virt_addr32++));
			//if((i+1) % 8 == 0)
			//	printf("\n");
		}
	//	printf("\n");
...
//				read_result = *((unsigned long *) virt_addr);
	...
	if(munmap(map_base, size) == -1) FATAL;
    close(fd);
    return 0;
}
